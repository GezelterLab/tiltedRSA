...
/****************************************************************************
 * This program is designed to look at a system of randomly aligned
 * "bent umbrellas" being dropped on an hcp lattice. The umbrellas
 * bind to the surface when they land within epsilon of the
 * interstitial points of the lattice. However, the landing will be
 * rejected if the umbrella overlaps a preexisting umbrella. The bent
 * umbrella structure is that of a circle with a handle attached to
 * the center of the circle. The circle is angled at the point of
 * attachment by the angle PHI, specified in the parameters.h. (NOTE:
 * the angle PHI needs to be between 2*pi and pi/2. Also the tests for
 * overlap do not work for the case where PHI is pi/2. For this case,
 * use the octopus code.)
 ****************************************************************************/
     ...
     char *argv[];
{
  ...
  int surface_scan = 0; /* boolean tells us whether to surface scan or no. */
  ...
  /**************************************************************************
   * The next pointer array is created to keep track of which umbrella
   * is in which bin. The first index is the x coordinate of the
   * bin. The second index is the y coordinate of the bin. The third
   * index is the number of the particle in the bin (first, second,
   * third, etc.)
   ***************************************************************************/
  ...
  while(success < MAXN){ /*break loop if we exceed maxn success*/
    ...
    /*pick the first two random points*/
    ...
    /*********************************************************************
     * This next section entails the check for the umbrella landing
     ...
     * 
     * With these equations, we can see if the random landing point
     * lies within epsilon of a lattice site. However, in practice,
     * once we calculate n and m with actual data points, they will be
     * double precision numbers. To these decimal numbers, we will add
     * 0.5 before casting them as integers. This will place the
     * nearest lattice point at the origin, and our point at some
     * distance away which will be less than half the distance to the
     * next lattice point.
     *
     **********************************************************************
     *
     ...
     * attachment to an interstitial point. These are tracked by the
     * spaces array. The array holds the template locations of the
     * interstitial points, which are then mapped to the nearest
     * lattice site.
     *
     **********************************************************************/
    ...
    if(lattice){
      ...
      n = (int)((temp.x / NND) + 0.5);
      m = (int)((temp.y / (sqrt3 * NND)) + 0.5);
      
      mapX = temp.x - n * NND;
      mapY = temp.y - m * sqrt3 * NND;
	...
	if(mapDsqr < epSqr){
	  ...
	  temp.x = n * NND + spaces[i].x;
	  temp.y = m * sqrt3 * NND + spaces[i].y;
	}
      }
      ...
      if(stick == 0){
	n = (int)(temp.x / NND);
	m = (int)(temp.y / (NND * sqrt3));
	
	mapX = temp.x - (NND * (2 * n + 1)) / 2;
	mapY = temp.y - (NND * sqrt3 * (2 * m + 1)) / 2;
	...
	for(i = 0; i < 6 && stick == 0; i++){
	  ...
	  if(mapDsqr < epSqr){
	    ...
	    temp.x = ((NND * (2 * n + 1)) / 2) + spaces[i].x;
	    temp.y = ((NND * sqrt3 * (2 * m + 1)) / 2) + spaces[i].y;
	    
	    /* map the point into the periodic box if necessary. */
    ...
    /*everybody automatically sticks */
    
    else{
      stick = 1;
    }
    
    /* if stick is true, set up a random orientation of the umbrella, and 
       determine Xbin and Ybin for the overlap test */
    
    if(stick){
      
      /* assign a random orientation to the dropped umbrella */
 ...
 exit: /* gives us a direct way out of all of the nested loops */
  ...
  /*if the surface scan is toggled, the coverage of the surface is
    scanned with a point probe. */
  ...
  /* if the surface scan was not toggled, then the true coverage will
     default to the continuous coverage. */
